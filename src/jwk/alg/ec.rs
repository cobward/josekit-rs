...
use anyhow::anyhow;
use elliptic_curve::pkcs8::{AssociatedOid, DecodePrivateKey, EncodePublicKey};
use elliptic_curve::sec1::{FromEncodedPoint, ModulusSize, ToEncodedPoint};
use elliptic_curve::{Curve, CurveArithmetic, JwkParameters, SecretKey};
    ...
    <C as CurveArithmetic>::AffinePoint: ToEncodedPoint<C>,
{
    ...
    pub(crate) fn into_private_key(self) -> SecretKey<C> {
        self.private_key
    }

    /// Generate EC key pair.
    pub fn generate() -> Result<EcKeyPair<C>, JoseError> {
        (|| -> anyhow::Result<EcKeyPair<C>> {
    ...
    pub fn from_der(input: impl AsRef<[u8]>) -> Result<Self, JoseError> {
        (|| -> anyhow::Result<Self> {
            ...
            let private_key = if Self::detect_pkcs8(input, false).is_some() {
                SecretKey::from_pkcs8_der(input).map_err(|e| anyhow!(e))?
            } else {
                SecretKey::from_sec1_der(input).map_err(|e| anyhow!(e))?
            };
...
impl<C> KeyPair for EcKeyPair<C>
where
    C: Curve + CurveArithmetic + JwkParameters + AssociatedOid,
    <C as elliptic_curve::Curve>::FieldBytesSize: ModulusSize,
    <C as CurveArithmetic>::AffinePoint: FromEncodedPoint<C>,
    <C as CurveArithmetic>::AffinePoint: ToEncodedPoint<C>,
{
    ...
    fn to_der_private_key(&self) -> Vec<u8> {
        self.private_key.to_sec1_der().unwrap().to_vec()
    }
    ...
    fn to_jwk_private_key(&self) -> Jwk {
        serde_json::from_str(&self.private_key.to_jwk_string()).unwrap()
    }

    fn to_jwk_public_key(&self) -> Jwk {
        serde_json::from_str(&self.private_key.public_key().to_jwk_string()).unwrap()
    }

    fn to_jwk_key_pair(&self) -> Jwk {
        self.to_jwk_private_key()
    }
...
impl<C: Curve> Deref for EcKeyPair<C>
where
    C: Curve + CurveArithmetic + JwkParameters + AssociatedOid,
    <C as elliptic_curve::Curve>::FieldBytesSize: ModulusSize,
    <C as CurveArithmetic>::AffinePoint: FromEncodedPoint<C>,
