...
use elliptic_curve::{Curve, CurveArithmetic, JwkParameters, PublicKey, SecretKey};
use sha2::Sha256;
...
impl EcdhEsJweAlgorithm {
    ...
    pub fn generate_ec_key_pair<C>(&self) -> Result<EcKeyPair<C>, JoseError>
    where
    ...
    //pub fn encrypter_from_der<C>(
    //    &self,
    //    input: impl AsRef<[u8]>,
    //) -> Result<EcdhEsJweEncrypter<C>, JoseError> {
    //    (|| -> anyhow::Result<EcdhEsJweEncrypter<C>> {
    ...
    //        let public_key = PublicKey::public_key_from_der(spki)?;
    ...
    pub fn encrypter_from_jwk<C>(&self, jwk: &Jwk) -> Result<EcdhEsJweEncrypter<C>, JoseError>
    where
        C: Curve + CurveArithmetic + JwkParameters + AssociatedOid,
        <C as elliptic_curve::Curve>::FieldBytesSize: ModulusSize,
        <C as CurveArithmetic>::AffinePoint: FromEncodedPoint<C>,
        <C as CurveArithmetic>::AffinePoint: ToEncodedPoint<C>,
    {
        (|| -> anyhow::Result<EcdhEsJweEncrypter<C>> {
            ...
            let (public_key, _key_type) = match jwk.parameter("crv") {
                Some(Value::String(val)) => match key_type {
                    "EC" => {
                        ...
                        let pkcs8 = EcKeyPair::<C>::to_pkcs8(&vec, true, curve);
                        let public_key =
                            PublicKey::from_public_key_der(&pkcs8).map_err(|e| anyhow!(e))?;

                        (public_key, EcdhEsKeyType::Ec(curve))
                    }
                    _ => bail!("unsupported keytype"),
                },
    ...
    pub fn decrypter_from_jwk<C>(&self, jwk: &Jwk) -> Result<EcdhEsJweDecrypter<C>, JoseError>
    where
        C: Curve + CurveArithmetic + JwkParameters + AssociatedOid,
        <C as elliptic_curve::Curve>::FieldBytesSize: ModulusSize,
        <C as CurveArithmetic>::AffinePoint: FromEncodedPoint<C>,
        <C as CurveArithmetic>::AffinePoint: ToEncodedPoint<C>,
    {
        (|| -> anyhow::Result<EcdhEsJweDecrypter<C>> {
            let key_type = match jwk.key_type() {
                val if val == "EC" || val == "OKP" => val,
                val => bail!("A parameter kty must be EC or OKP: {}", val),
            };
    ...
    fn concat_kdf(
        ...
        let count = util::ceiling(shared_key_len, <Sha256 as Digest>::output_size());
        for i in 0..count {
...
#[derive(Debug, Clone)]
pub struct EcdhEsJweEncrypter<C: CurveArithmetic> {
...
impl<C> JweEncrypter for EcdhEsJweEncrypter<C>
where
    C: Curve + CurveArithmetic + JwkParameters + AssociatedOid,
    <C as elliptic_curve::Curve>::FieldBytesSize: ModulusSize,
    <C as CurveArithmetic>::AffinePoint: FromEncodedPoint<C>,
...
impl<C> Deref for EcdhEsJweEncrypter<C>
where
    C: Curve + CurveArithmetic + JwkParameters + AssociatedOid,
    <C as elliptic_curve::Curve>::FieldBytesSize: ModulusSize,
    <C as CurveArithmetic>::AffinePoint: FromEncodedPoint<C>,
...
#[derive(Debug, Clone)]
pub struct EcdhEsJweDecrypter<C: Curve> {
    algorithm: EcdhEsJweAlgorithm,
    private_key: SecretKey<C>,
    _key_type: EcdhEsKeyType,
    key_id: Option<String>,
}

impl<C: Curve> EcdhEsJweDecrypter<C> {
    pub fn set_key_id(&mut self, value: impl Into<String>) {
        self.key_id = Some(value.into());
    }
...
impl<C> JweDecrypter for EcdhEsJweDecrypter<C>
where
    ...
    <C as CurveArithmetic>::AffinePoint: ToEncodedPoint<C>,
{
    ...
    fn decrypt(
        ...
        (|| -> anyhow::Result<Cow<[u8]>> {
            ...
            let shared_secret: SharedSecret<C> = elliptic_curve::ecdh::diffie_hellman(
...
impl<C> Deref for EcdhEsJweDecrypter<C>
where
    C: Curve + CurveArithmetic + JwkParameters,
    <C as elliptic_curve::Curve>::FieldBytesSize: ModulusSize,
    <C as CurveArithmetic>::AffinePoint: FromEncodedPoint<C>,
